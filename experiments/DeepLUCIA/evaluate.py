from train import SeqEpiModel, load_data, load_data_by_health
...
model = SeqEpiModel()
checkpoint = torch.load(r'./model/LoopModel_2023_08_10_18_56_11_8cellbyhealth.pt')
...
# _, _, _, val_seqs, val_hs, val_labels = load_data(cell_line_list=cell_line_list, val_chr_list=VAL_CHR_LIST)
_, _, _, val_seqs, val_hs, val_labels = load_data_by_health(cell_line_list=cell_line_list, val_cell_line_list=val_cell_line_list)
print("="*7)
log_info = f"""len(train_features):
len(train_hs):
len(train_labels):
...
BATCH_SIZE = 256
val_seqs = torch.tensor(val_seqs, dtype=torch.float32).cpu()
val_hs = torch.tensor(val_hs, dtype=torch.float32).cpu()
val_labels = torch.tensor(val_labels, dtype=torch.float32).cpu()
test_pred_scores = model(val_seqs[:BATCH_SIZE].cuda(), val_hs[:BATCH_SIZE].cuda()).cpu()
with torch.no_grad():
    for i in range(1, math.ceil(len(val_labels)/BATCH_SIZE)):
        print(i)
        vstemp = val_seqs[i*BATCH_SIZE:(i+1)*BATCH_SIZE].cuda()
        vhtemp = val_hs[i*BATCH_SIZE:(i+1)*BATCH_SIZE].cuda()
        print("1", vstemp.shape)
        print("2", vhtemp.shape)
        temp = model(vstemp, vhtemp)
        temp = temp.cpu()
