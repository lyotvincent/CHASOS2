...
import random
import numpy as np
from parameters import FINE_TUNED_ANCHOR_MODEL_PATH, PRETRAINED_MODEL_PATH
from pretrained_model.pretrained_data_loader import load_kmer_encoded_data, PretrainedDataset
from anchor_models import *
import torch
from torch.utils.data import DataLoader
from torch.utils.tensorboard.writer import SummaryWriter
import torch.optim.lr_scheduler as lr_scheduler
from metrics import compute_one_loss, compute_one_acc
...
log_out = open(FINE_TUNED_ANCHOR_MODEL_PATH+r'/logs/log_'+log_time+'.txt', 'w', buffering=8)
train_writer = SummaryWriter(FINE_TUNED_ANCHOR_MODEL_PATH+f'/logs/tensorboard_log_{log_time}/train')
val_writer = SummaryWriter(FINE_TUNED_ANCHOR_MODEL_PATH+f'/logs/tensorboard_log_{log_time}/val')
...
saved_dict['out_layer.4.weight'] = saved_dict['out_layer.4.weight'][0:1, :]
saved_dict['out_layer.4.bias'] = saved_dict['out_layer.4.bias'][0:1]
...
for e in range(EPOCH):
    ...
    for batch_num, batch in enumerate(train_dataloader):
        ...
        # 计算损失函数
        loss = compute_one_loss(pred_scores, torch.unsqueeze(batch_labels[:, 0], dim=1))
        # 反向传播
        loss.backward()
        # 更新梯度
        optimizer.step()
        ...
        if batch_num % LOG_STEP == 0:
            model.eval()
            with torch.no_grad():
                anchor_acc = compute_one_acc(pred_scores, torch.unsqueeze(batch_labels[:, 0], dim=1))
                ...
                for val_batch in val_dataloader:
                    ...
                    val_loss += compute_one_loss(val_pred_scores, torch.unsqueeze(val_batch_labels[:, 0], dim=1)).item()
                    temp_val_anchor_acc = compute_one_acc(val_pred_scores, torch.unsqueeze(val_batch_labels[:, 0], dim=1))
                ...
                if val_anchor_acc > best_val_anchor_acc: # update best val anchor acc
                    ...
                    torch.save(checkpoint,FINE_TUNED_ANCHOR_MODEL_PATH+f'/model/AnchorModel_{log_time}.pt')
            # 早停止
