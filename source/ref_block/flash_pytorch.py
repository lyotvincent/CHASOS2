...
class ScaleNorm(nn.Module):
    def __init__(self, dim, eps = 1e-5):
        ...
        self.scale = dim ** -0.5
        self.eps = eps
        self.g = nn.Parameter(torch.ones(1)) # type: ignore
...
class OffsetScale(nn.Module):
    def __init__(self, dim, heads = 1):
        super().__init__()
        self.gamma = nn.Parameter(torch.ones(heads, dim)) # type: ignore
        self.beta = nn.Parameter(torch.zeros(heads, dim)) # type: ignore
...
class GAU(nn.Module):
    ...
    def forward(
        ...
        if exists(mask):
            ...
            attn = attn.masked_fill(~mask, 0.) # type: ignore
    ...
    def forward(
        ...
        *,
        ...
        if exists(mask):
            ...
            lin_k = lin_k.masked_fill(~lin_mask, 0.) # type: ignore

        # rotate queries and keys

        if exists(self.rotary_pos_emb):
            quad_q, lin_q, quad_k, lin_k = map(self.rotary_pos_emb.rotate_queries_or_keys, (quad_q, lin_q, quad_k, lin_k)) # type: ignore
...
class FLASHTransformer(nn.Module):
    def __init__(
        ...
        *,
        ...
        num_tokens, # embedding dictionary size
        depth,
        group_size = 256,
        query_key_dim = 128,
        expansion_factor = 2.,
